(ns mindmaze.main
  "Kotodama WASM entrypoint for the Mindmaze clean-room actor (L4) — Clojure port.
  ...
  facts (`mindmaze.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mindmaze")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "mindmaze_dev"
    :fields [:model :channels :samplingHz :firmware] :required [:model :channels]
    :coerce {:channels :int :samplingHz :int} :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "mindmaze_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"mindmaze.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mindmaze-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mindmaze-compat/ready)
